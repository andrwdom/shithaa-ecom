...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://127.0.0.1:27017/shitha';
...
const fixLoungewearOffer = async () => {
  try {
    ...
    // Find all products that should be loungewear based on their names
    const allProducts = await productModel.find({});
    console.log(`📊 Total products in database: ${allProducts.length}`);
    
    const potentialLoungewearProducts = allProducts.filter(product => {
      const name = product.name.toLowerCase();
      return name.includes('lounge') || 
             name.includes('loungwear') || 
             (name.includes('feeding') && !name.includes('maternity')) ||
             name.includes('zipless');
    });
    ...
    for (const product of potentialLoungewearProducts) {
      ...
      if (name.includes('zipless') && name.includes('feeding') && name.includes('lounge')) {
        ...
        // This could be either zipless-feeding-lounge-wear or non-feeding-lounge-wear
        // Let's default to zipless-feeding-lounge-wear if it mentions feeding
        if (product.categorySlug !== 'zipless-feeding-lounge-wear') {
          newCategorySlug = 'zipless-feeding-lounge-wear';
          needsUpdate = true;
        }
      } else if (name.includes('maternity') && name.includes('feeding')) {
        if (product.categorySlug !== 'maternity-feeding-wear') {
          newCategorySlug = 'maternity-feeding-wear';
          needsUpdate = true;
        }
      } else if (name.includes('dupatta') && name.includes('lounge')) {
        if (product.categorySlug !== 'zipless-feeding-dupatta-lounge-wear') {
          newCategorySlug = 'zipless-feeding-dupatta-lounge-wear';
          needsUpdate = true;
        }
      }
    ...
    // Step 2: Verify loungewear products for offer
    console.log('🔧 Step 2: Verifying loungewear products for offer...');
    
    const loungewearProducts = await productModel.find({
      categorySlug: { $in: ['zipless-feeding-lounge-wear', 'non-feeding-lounge-wear'] }
    });
    
    console.log(`📊 Found ${loungewearProducts.length} loungewear products eligible for offer:`);
    ...
    if (loungewearProducts.length >= 3) {
      ...
      if (exactResult.discount === expectedDiscount) {
        console.log('\n🎉 SUCCESS: The offer calculation is working correctly!');
        console.log('   When 3 loungewear items @ ₹450 each are in cart:');
        console.log('   - Original total: ₹1350');
        console.log('   - Offer price: ₹1299 (3 for ₹1299)');
        console.log('   - Discount: ₹51');
      } else {
        console.log('\n❌ ISSUE: The offer calculation is not working as expected.');
      }
    ...
    console.log(`   - Found ${loungewearProducts.length} loungewear products eligible for offer`);
    console.log(`   - Offer logic: 3+ loungewear items get "3 for ₹1299" pricing`);
    console.log(`   - For 3 items @ ₹450 each: ₹1350 → ₹1299 (₹51 discount)`);
