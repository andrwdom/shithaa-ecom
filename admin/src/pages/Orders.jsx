    ...
    if (loading) {
      return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from({ length: 6 }).map((_, index) => <OrderSkeleton key={index} />)}
        </div>
      );
    }
    ...
    if (error) {
      return (
        <div className="text-center py-16 bg-red-50 text-red-700 rounded-lg">
          ...
          <button onClick={fetchOrders} className="mt-6 px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700">
            Try Again
          </button>
        </div>
      );
        ...
        {orders.map(order => (
          <div key={order._id} className="bg-white p-4 rounded-lg shadow">
            ...
            <div className="flex justify-between items-center">
              ...
              <select value={order.status} onChange={(e) => updateOrderStatus(order._id, e.target.value)} className="border rounded px-2 py-1 text-sm">
